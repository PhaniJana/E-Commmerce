...
const Add = ({token}) => {
  ...
  const onSubmitHandler= async (e)=>{
    ...
    try {
      ...
      formData.append('sizes',JSON.stringify(sizes))  //cannot send array so stringify
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        ...
        <textarea onChange={(e)=>setDesription(e.target.value)} value={desription} className='w-full max-w-[500px] px-3 py-2 ' type="text" placeholder='Enter Description' required />
      </div>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          ...
          <select className='w-full px-3 py-2' onChange={(e)=>setSubCategory(e.target.value)}>
            <option vlaue='Topwear'>Topwear</option>
            <option vlaue='Bottomwear'>Bottomwear</option>
            <option vlaue='Winterwear'>Winterwear</option>
          </select>
        </div>

        <div>
          <p className='mb-2'>Product Price</p>
          <input className='w-full px-3 py-2 sm:w-[120px]' type="number" onChange={(e)=>setPrice(e.target.value)} value={price} placeholder='25'/>
        </div>
      </div>
